...
import { pageStudent, searchStudent, createStudent, updateStudent, deleteStudent } from '@/api/student'
import { getTeacherClasses } from '@/api/public'
...
const searchResults = ref<StudentRow[]>([])
...
async function loadData() {
  loading.value = true
  try {
    const res: any = await pageStudent(currentPage.value, pageSize.value)
...
async function handleSearch() {
  ...
  try {
    ...
    const res: any = await searchStudent(params)
...
async function handleDelete(row: StudentRow) {
  await ElMessageBox.confirm(`确认删除学生「${row.name}」？`, '提示', {
